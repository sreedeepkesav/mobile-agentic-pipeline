...
Complete reference for the Bootstrap stage of the ios-builder skill. Covers context detection, MCP discovery, spec assessment, project state detection, and stage configuration.
...
**Full Reconfigure:**
```bash
ios-builder --reconfigure
```
Re-runs all 5 bootstrap steps. Updates `.pipeline.yml`.

**Single Step Reconfigure:**
```bash
ios-builder --reconfigure context
# Re-run only context detection
```

**Other Single Steps:**
```bash
ios-builder --reconfigure mcp_discovery
ios-builder --reconfigure spec_assessment
ios-builder --reconfigure project_state
ios-builder --reconfigure stages
```
